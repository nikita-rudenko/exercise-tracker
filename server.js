...
const UserModel = mongoose.model('UserModel', new Schema({user:{type: String, required: true}}));

// Exercise Model
const ExModel = mongoose.model('ExModel', new Schema({
...
app.get('/api/exercise/log?:userId/:from?/:to?/:limit?', function(req, res){
  ...
  if(!userId){res.end("Error! userId required")};
  ...
  // Search results
  ExModel.find(search).limit(limit).then((ex) => {
